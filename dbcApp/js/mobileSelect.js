...
window.MobileSelect = (function() {
	...
	function MobileSelect(config) {
		this.mobileSelect;
		this.wheelsData = config.wheels;  //市场数据
		this.jsonType =  false;
		this.jsonData = [];
		this.checkDataType();   //判断数据是否为json格式
		this.renderWheels(this.wheelsData);
		this.displayJson = []; 
		this.cascade = false;
		this.startY;       //手指触摸屏幕时的开始位置
		this.moveEndY;     //手指离开屏幕时的位置
		this.moveY;        //手指拖曳元素时的当前位置
		this.oldMoveY;     //手指拖曳元素前的位置
		this.offset = 0;   //手指拖曳元素的位移距离
		this.offsetSum = 0;//手指触摸屏幕和离开屏幕的位移差
		this.oversizeBorder;
		this.curDistance = [];
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			...
			_this.market = document.querySelector('.marketName');
		    ...
		    _this.ensureBtn.addEventListener('click',function(){
		    	...
		    	/*for(var i=0; i<_this.wheel.length; i++){
		    		...
		    		tempId = _this.getId(i);
		    	}*/
		    	tempValue += _this.getValue(0);
		    	tempId = _this.getId(0);
		    	
		    	_this.market.innerHTML = tempValue;
		    	_this.market.id = tempId;
	    ...
	    getId: function(sliderIndex){
	    	var _this = this;
	    	var index = _this.getIndex(_this.curDistance[sliderIndex]);
	    	console.log(_this.slider[sliderIndex].getElementsByTagName('li')[index].getAttribute("data-id"));
	    ...
	    touch: function(event, theSlider, index){
	    	...
	    	switch(event.type){
	    		case "touchstart":  //touchstart：手指放在一个DOM元素上
			        _this.startY = event.touches[0].clientY;
			        _this.oldMoveY = _this.startY;
	    			break;

	    		case "touchend":  //touchend：手指从一个DOM元素上移开
	    		...
	    		case "touchmove":  //touchmove：手指拖曳一个DOM元素
